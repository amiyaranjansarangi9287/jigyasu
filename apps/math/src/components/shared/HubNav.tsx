    ...
    <div className="mb-6">
      ...
      <div className={`${useAccordion ? 'hidden sm:flex' : 'flex'} flex-wrap justify-center gap-1.5`}>
        {items.map(m => (
          <motion.button
            key={m.id}
            className={`relative px-2.5 sm:px-3.5 py-2 rounded-xl font-medium text-xs sm:text-sm transition-all whitespace-nowrap ${
      ...
      {useAccordion && (
        <div className="sm:hidden space-y-2">
          ...
          <div className="flex items-center justify-between bg-white/5 rounded-xl px-4 py-2 border border-white/10">
            ...
            <span className="text-gray-500 text-xs">tap below to switch</span>
          </div>
