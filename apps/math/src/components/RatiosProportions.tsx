          ...
          <motion.div key="r" className="max-w-lg mx-auto space-y-4" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
              ...
              <div className="flex h-10 rounded-lg overflow-hidden">
                <motion.div className="bg-blue-500 flex items-center justify-center text-xs font-bold text-white" animate={{ width: `${(a / (a + b)) * 100}%` }}>{a}</motion.div>
                <motion.div className="bg-orange-500 flex items-center justify-center text-xs font-bold text-white" animate={{ width: `${(b / (a + b)) * 100}%` }}>{b}</motion.div>
              </div>
              <div className="flex justify-between text-xs mt-1"><span className="text-blue-400">A: {((a / (a + b)) * 100).toFixed(1)}%</span><span className="text-orange-400">B: {((b / (a + b)) * 100).toFixed(1)}%</span></div>
            </div>
        ...
        {mode === 'recipe' && (
          <motion.div key="re" className="max-w-lg mx-auto space-y-4" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="bg-gradient-to-br from-orange-500/10 to-amber-500/10 rounded-2xl p-5 border border-orange-500/20">
              ...
              <p className="text-xs text-gray-400 mb-3">Scale factor: <span className="text-orange-300 font-bold">×{scale.toFixed(2)}</span></p>
