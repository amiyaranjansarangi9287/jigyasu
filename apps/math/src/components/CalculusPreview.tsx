    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-4">
        <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
          ...
          <div className="flex items-center gap-3"><label className="text-gray-400 text-xs">x →</label><input type="range" min="-4" max="4" step="0.1" value={xVal} onChange={e => setXVal(Number(e.target.value))} className="flex-1 accent-yellow-500" /><span className="text-yellow-400 font-bold w-10">{xVal}</span></div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20 text-center"><p className="text-gray-400 text-xs">f(x) = {fLabel}</p><motion.p key={yVal} className="text-purple-400 font-bold text-xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{yVal.toFixed(3)}</motion.p></div>
          <div className="bg-red-500/10 rounded-xl p-4 border border-red-500/20 text-center"><p className="text-gray-400 text-xs">f'(x) = {dfLabel}</p><motion.p key={slope} className="text-red-400 font-bold text-xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{slope.toFixed(3)}</motion.p></div>
        </div>
      ...
      <div className="space-y-4">
        <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3"><label className="text-blue-400 text-xs w-6">a</label><input type="range" min="-4" max="4" step="0.1" value={aVal} onChange={e => setAVal(Math.min(Number(e.target.value), bVal - 0.1))} className="flex-1 accent-blue-500" /><span className="text-blue-400 font-bold w-10">{aVal}</span></div>
            <div className="flex items-center gap-3"><label className="text-orange-400 text-xs w-6">b</label><input type="range" min="-4" max="4" step="0.1" value={bVal} onChange={e => setBVal(Math.max(Number(e.target.value), aVal + 0.1))} className="flex-1 accent-orange-500" /><span className="text-orange-400 font-bold w-10">{bVal}</span></div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-green-500/10 rounded-xl p-4 border border-green-500/20 text-center"><p className="text-gray-400 text-xs">Approx area ({numRects} rects)</p><motion.p key={approxArea} className="text-green-400 font-bold text-xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{approxArea.toFixed(3)}</motion.p></div>
          <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20 text-center"><p className="text-gray-400 text-xs">Exact ∫x² dx</p><motion.p key={exactArea} className="text-purple-400 font-bold text-xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{exactArea.toFixed(3)}</motion.p></div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 border border-white/10 text-sm text-gray-300 space-y-1">
