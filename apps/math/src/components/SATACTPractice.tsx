      ...
      {!started ? (
            ...
            {questions.map((q, i) => {
              ...
              return (
                <div key={i} className={`rounded-xl p-4 border ${correct ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                  <div className="flex items-start gap-2"><span>{correct ? '✅' : '❌'}</span><div><p className="text-white text-sm">{q.question}</p>{!correct && <p className="text-green-400 text-xs mt-1">Correct: {q.answer}</p>}<p className="text-gray-400 text-xs mt-1">{q.explanation}</p></div></div>
                </div>
              );
            })}
          </div>
        ...
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <div className="rounded-3xl p-6 bg-white/5 border border-white/10">
            <div className="flex gap-2 mb-3"><span className="text-xs bg-purple-500/30 text-purple-300 px-2 py-0.5 rounded-full">{q.category}</span><span className={`text-xs px-2 py-0.5 rounded-full ${q.difficulty === 'Easy' ? 'bg-green-500/30 text-green-300' : q.difficulty === 'Medium' ? 'bg-yellow-500/30 text-yellow-300' : 'bg-red-500/30 text-red-300'}`}>{q.difficulty}</span></div>
            ...
            <div className="space-y-2">
              {q.options.map((opt, i) => (
                <motion.button key={i} className={`w-full py-3 px-4 rounded-xl text-left font-bold text-sm flex items-center gap-3 ${answers[current] === opt ? 'bg-purple-500/30 border-2 border-purple-400' : 'bg-white/10 border border-white/20 hover:bg-white/20'}`}
                  ...
                  <span className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-xs text-gray-400">{String.fromCharCode(65 + i)}</span>
          ...
          {/* Question navigator */}
          <div className="flex justify-center gap-1">{questions.map((_, i) => <button key={i} className={`w-8 h-8 rounded-lg text-xs font-bold ${i === current ? 'bg-purple-500 text-white' : answers[i] ? 'bg-green-500/30 text-green-300' : 'bg-white/10 text-gray-500'}`} onClick={() => setCurrent(i)}>{i + 1}</button>)}</div>
        </div>
      ) : null}
    </div>
  );
