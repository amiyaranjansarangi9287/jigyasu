          ...
          <motion.div key="ex" className="grid grid-cols-1 lg:grid-cols-2 gap-6" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="space-y-4">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <div className="grid grid-cols-2 gap-2"><div><label className="text-gray-400 text-xs">Real</label><input type="range" min="-6" max="6" value={a1} onChange={e => setA1(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{a1}</p></div><div><label className="text-gray-400 text-xs">Imaginary</label><input type="range" min="-6" max="6" value={b1} onChange={e => setB1(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{b1}i</p></div></div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <h4 className="text-orange-400 font-bold mb-2">z₂ = {fmt(a2, b2)}</h4>
                <div className="grid grid-cols-2 gap-2"><div><label className="text-gray-400 text-xs">Real</label><input type="range" min="-6" max="6" value={a2} onChange={e => setA2(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{a2}</p></div><div><label className="text-gray-400 text-xs">Imaginary</label><input type="range" min="-6" max="6" value={b2} onChange={e => setB2(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{b2}i</p></div></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20 text-center"><p className="text-gray-400 text-xs">z₁ + z₂</p><p className="text-green-400 font-bold font-mono">{fmt(results.sumR, results.sumI)}</p></div>
                <div className="bg-purple-500/10 rounded-xl p-3 border border-purple-500/20 text-center"><p className="text-gray-400 text-xs">z₁ × z₂</p><p className="text-purple-400 font-bold font-mono">{fmt(results.mulR, results.mulI)}</p></div>
                <div className="bg-cyan-500/10 rounded-xl p-3 border border-cyan-500/20 text-center"><p className="text-gray-400 text-xs">|z₁|</p><p className="text-cyan-400 font-bold">{results.mag1.toFixed(2)}</p></div>
                <div className="bg-pink-500/10 rounded-xl p-3 border border-pink-500/20 text-center"><p className="text-gray-400 text-xs">z̄₁ (conjugate)</p><p className="text-pink-400 font-bold font-mono">{fmt(results.conjR, results.conjI)}</p></div>
              </div>
