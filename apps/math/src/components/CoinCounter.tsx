  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-white/5 rounded-2xl p-5 border border-white/10 mb-4">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {COINS.map(c => (
            <motion.button key={c.value}
              ...
              <p className="text-white/70 text-xs">{c.value}¢</p>
              {(counts[c.value] || 0) > 0 && (
                <motion.span className="absolute -top-2 -right-2 w-6 h-6 bg-white text-black rounded-full text-xs font-bold flex items-center justify-center"
        ...
        {totalCoins > 0 && (
          <div className="flex items-center justify-between mt-4 bg-black/20 rounded-xl px-4 py-3">
            ...
            <button className="text-xs text-red-400 hover:text-red-300 ml-4" onClick={clear}>Clear</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-2xl p-5 border border-green-500/30 text-center mb-4">
        ...
        <p className="text-gray-500 text-xs mt-1">{totalCoins} coin{totalCoins !== 1 ? 's' : ''}</p>
      </div>
