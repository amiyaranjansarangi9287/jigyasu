    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-3">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
          <div><label className="text-gray-400 text-xs">Span (m)</label><input type="range" min="5" max="50" value={span} onChange={e => setSpan(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{span} m</p></div>
          <div><label className="text-gray-400 text-xs">Load (N)</label><input type="range" min="100" max="2000" step="50" value={load} onChange={e => setLoad(Number(e.target.value))} className="w-full accent-red-500" /><p className="text-white font-bold text-center">{load} N</p></div>
          <div><label className="text-gray-400 text-xs">Beam height (m)</label><input type="range" min="0.5" max="5" step="0.5" value={beamHeight} onChange={e => setBeamHeight(Number(e.target.value))} className="w-full accent-yellow-500" /><p className="text-white font-bold text-center">{beamHeight} m</p></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-orange-500/10 rounded-xl p-3 border border-orange-500/20 text-center"><p className="text-gray-400 text-xs">Max Bending</p><motion.p key={maxBending} className="text-orange-400 font-bold text-lg" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{maxBending.toFixed(0)} Nm</motion.p></div>
          <div className={`rounded-xl p-3 border text-center ${stress > 100000 ? 'bg-red-500/20 border-red-500/30' : 'bg-green-500/10 border-green-500/20'}`}><p className="text-gray-400 text-xs">Stress</p><motion.p key={stress} className={`font-bold text-lg ${stress > 100000 ? 'text-red-400' : 'text-green-400'}`} initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{(stress / 1000).toFixed(1)} kPa</motion.p></div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 border border-white/10 text-xs text-gray-300">
...
function ProjectileMotion() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-3">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
          <div><label className="text-gray-400 text-xs">Initial velocity (m/s)</label><input type="range" min="5" max="50" value={velocity} onChange={e => setVelocity(Number(e.target.value))} className="w-full accent-purple-500" /><p className="text-white font-bold text-center">{velocity} m/s</p></div>
          <div><label className="text-gray-400 text-xs">Launch angle (°)</label><input type="range" min="5" max="85" value={angle} onChange={e => setAngle(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{angle}°</p></div>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-blue-500/10 rounded-xl p-3 border border-blue-500/20 text-center"><p className="text-gray-400 text-xs">Range</p><motion.p key={range} className="text-blue-400 font-bold" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{range.toFixed(1)} m</motion.p></div>
          <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20 text-center"><p className="text-gray-400 text-xs">Max Height</p><motion.p key={maxHeight} className="text-green-400 font-bold" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{maxHeight.toFixed(1)} m</motion.p></div>
          <div className="bg-orange-500/10 rounded-xl p-3 border border-orange-500/20 text-center"><p className="text-gray-400 text-xs">Flight Time</p><motion.p key={timeOfFlight} className="text-orange-400 font-bold" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{timeOfFlight.toFixed(2)} s</motion.p></div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 border border-white/10 text-xs text-gray-300 space-y-1">
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
        ...
        <div className="space-y-3">
          <div><label className="text-gray-400 text-xs">Target Volume (cm³)</label><input type="range" min="100" max="5000" step="100" value={volume} onChange={e => setVolume(Number(e.target.value))} className="w-full accent-purple-500" /><p className="text-white font-bold text-center">{volume} cm³</p></div>
          <div><label className="text-gray-400 text-xs">Length (cm)</label><input type="range" min="1" max="30" step="0.5" value={length} onChange={e => setLength(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{length} cm</p></div>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-blue-500/10 rounded-xl p-3 border border-blue-500/20 text-center"><p className="text-gray-400 text-xs">L × W × H</p><p className="text-blue-400 font-bold text-sm">{length.toFixed(1)} × {width.toFixed(1)} × {height.toFixed(1)}</p></div>
        <div className="bg-orange-500/10 rounded-xl p-3 border border-orange-500/20 text-center"><p className="text-gray-400 text-xs">Surface Area</p><p className="text-orange-400 font-bold">{surfaceArea.toFixed(0)} cm²</p></div>
        <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20 text-center"><p className="text-gray-400 text-xs">Optimal (cube)</p><p className="text-green-400 font-bold">{optimalSA.toFixed(0)} cm²</p></div>
        <div className={`rounded-xl p-3 border text-center ${efficiency > 90 ? 'bg-green-500/20 border-green-500/30' : 'bg-yellow-500/10 border-yellow-500/20'}`}><p className="text-gray-400 text-xs">Efficiency</p><p className={`font-bold ${efficiency > 90 ? 'text-green-400' : 'text-yellow-400'}`}>{efficiency}%</p></div>
      </div>
