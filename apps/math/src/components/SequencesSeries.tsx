  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-white mb-2">∑ Sequences & Series</h2>
        <p className="text-purple-300 text-lg">Explore arithmetic and geometric sequences, sums, and sigma notation!</p>
      </div>

      <div className="flex justify-center gap-2 mb-4">
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${seqType === 'arithmetic' ? 'bg-blue-500/30 text-blue-300 border border-blue-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => setSeqType('arithmetic')}>📏 Arithmetic</button>
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${seqType === 'geometric' ? 'bg-purple-500/30 text-purple-300 border border-purple-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => setSeqType('geometric')}>📈 Geometric</button>
      </div>
      <div className="flex justify-center gap-2 mb-6">
        ...
        <button className={`px-4 py-2 rounded-xl text-sm font-bold ${mode === 'challenge' ? 'bg-orange-500/30 text-orange-300' : 'bg-white/5 text-gray-400'}`} onClick={() => { setMode('challenge'); setChallenge(makeChallenge()); }}>🎯 Challenge</button>
      </div>
          ...
          <motion.div key="ex" className="max-w-3xl mx-auto space-y-4" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div><label className="text-gray-400 text-xs">First term (a₁)</label><input type="range" min="1" max="10" value={a1} onChange={e => setA1(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{a1}</p></div>
                {seqType === 'arithmetic' ? (
                  <div><label className="text-gray-400 text-xs">Common diff (d)</label><input type="range" min="-5" max="10" value={d} onChange={e => setD(Number(e.target.value))} className="w-full accent-green-500" /><p className="text-white font-bold text-center">{d}</p></div>
                ) : (
                  <div><label className="text-gray-400 text-xs">Common ratio (r)</label><input type="range" min="-3" max="5" value={r} onChange={e => setR(Number(e.target.value))} className="w-full accent-purple-500" /><p className="text-white font-bold text-center">{r}</p></div>
                )}
                <div><label className="text-gray-400 text-xs">Terms (n)</label><input type="range" min="3" max="15" value={n} onChange={e => setN(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{n}</p></div>
              </div>
            </div>

            {/* Sequence display */}
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
              <h4 className="text-white font-bold mb-2">Sequence</h4>
              ...
              <p className="text-gray-400 text-sm">Sigma notation: <span className="text-white font-mono">∑ᵢ₌₁ⁿ {seqType === 'arithmetic' ? `(${a1} + (i-1)·${d})` : `${a1}·${r}ⁱ⁻¹`}</span></p>
            </div>
              ...
              <div className="flex items-end gap-1 min-h-32">
                {terms.map((t, i) => (
                  <motion.div key={i} className="flex-1 flex flex-col items-center"
                    ...
                    <span className="text-[10px] text-gray-400 mb-0.5">{Math.round(t)}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="bg-green-500/10 rounded-xl p-4 border border-green-500/20 text-center"><p className="text-gray-400 text-xs">Sum of {n} terms</p><motion.p key={sum} className="text-green-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{Math.round(formulaSum * 100) / 100}</motion.p></div>
              <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20 text-center"><p className="text-gray-400 text-xs">nth term (a{toSup(n)})</p><motion.p key={nthTerm} className="text-blue-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{Math.round(nthTerm * 100) / 100}</motion.p></div>
              <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20 text-center"><p className="text-gray-400 text-xs">Average</p><motion.p key={sum / n} className="text-purple-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{(sum / n).toFixed(2)}</motion.p></div>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-4 border border-white/10 text-sm text-gray-300 space-y-1">
              {seqType === 'arithmetic' ? (
                <>
                  <p>📝 <strong>nth term:</strong> aₙ = a₁ + (n-1)d = {a1} + ({n}-1)·{d} = {nthTerm}</p>
                  <p>📝 <strong>Sum:</strong> Sₙ = n/2 · (2a₁ + (n-1)d) = {n}/2 · (2·{a1} + ({n}-1)·{d}) = {formulaSum}</p>
                </>
              ) : (
                <>
                  <p>📝 <strong>nth term:</strong> aₙ = a₁ · r^(n-1) = {a1} · {r}{toSup(n - 1)} = {Math.round(nthTerm * 100) / 100}</p>
                  <p>📝 <strong>Sum:</strong> Sₙ = a₁(rⁿ - 1)/(r - 1) = {a1}({r}{toSup(n)} - 1)/({r} - 1) = {Math.round(formulaSum * 100) / 100}</p>
                </>
              )}
            </div>
          </motion.div>
