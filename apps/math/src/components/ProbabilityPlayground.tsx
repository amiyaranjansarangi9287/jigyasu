    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="space-y-4">
        ...
        <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20">
          ...
          <p className="text-gray-400 text-xs mt-1">Each flip is independent — past results don't affect future flips!</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          <div className="flex h-8 rounded-lg overflow-hidden">
            <motion.div className="bg-yellow-500 flex items-center justify-center text-xs font-bold"
              animate={{ width: `${stats.total > 0 ? (stats.heads / stats.total) * 100 : 50}%` }}
              transition={{ duration: 0.3 }}>
              {stats.headsPercent}%
            </motion.div>
            <motion.div className="bg-gray-500 flex items-center justify-center text-xs font-bold"
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="space-y-4">
        ...
        <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20">
          ...
          <p className="text-gray-400 text-xs mt-1">Expected average = 3.5</p>
        </div>
      </div>
              ...
              return (
                <div key={n} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-6 bg-gray-700 rounded-full overflow-hidden">
                    <motion.div
                      className="h-full bg-gradient-to-r from-red-500 to-rose-500 rounded-full flex items-center justify-end pr-2 text-xs font-bold"
                      animate={{ width: `${Math.max(pct, 2)}%` }}
                    >
                      {stats.freq[n]}
                    </motion.div>
                  </div>
                  <span className="text-gray-400 text-xs w-12">{pct.toFixed(1)}%</span>
                </div>
              );
            })}
          </div>
