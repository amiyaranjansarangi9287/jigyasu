          ...
          <div className="flex gap-1">
            {[12, 24, 36, 60, 100, 180, 360].map(n => (
              <button key={n} onClick={() => setNum(n)}
                className={`px-2 py-1 rounded text-xs ${num === n ? 'bg-purple-500/40 text-purple-300' : 'bg-white/10 text-gray-400'}`}
              >{n}</button>
            ))}
          </div>
        </div>
          ...
          {treeNodes.map((level, li) => (
            ...
            >
              {level.map((node, ni) => (
                ...
                >
                  {node.value}
                  {node.isPrime && node.value > 1 && <span className="ml-1 text-xs">★</span>}
                </motion.div>
              ))}
              {li < treeNodes.length - 1 && (
                <div className="w-full text-center text-gray-500 text-xs">↓</div>
              )}
            </motion.div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          ...
          <div className="flex flex-wrap gap-1">
            {factors.map(f => (
              <span key={f} className={`px-2 py-1 rounded text-xs font-bold ${isPrime(f) ? 'bg-yellow-500/20 text-yellow-300' : 'bg-white/10 text-gray-300'}`}>
                {f}
              </span>
            ))}
          </div>
    ...
    <div className="space-y-4">
      <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <div className="flex flex-col items-center gap-1">
            <label className="text-blue-400 text-xs font-bold">Number A</label>
          ...
          <div className="flex flex-col items-center gap-1">
            <label className="text-orange-400 text-xs font-bold">Number B</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-2xl p-5 border border-green-500/20">
          ...
          <p className="text-gray-400 text-xs mt-2">The largest number that divides both {a} and {b}</p>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-indigo-500/10 rounded-2xl p-5 border border-blue-500/20">
          ...
          <p className="text-gray-400 text-xs mt-2">The smallest number divisible by both {a} and {b}</p>
      ...
      <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
        ...
        <div className="flex flex-wrap gap-1 justify-center">
          {factorsA.map(f => {
            ...
            return (
              <motion.span key={`a-${f}`}
                className={`px-2 py-1 rounded text-xs font-bold ${isCommon ? 'bg-green-500/30 text-green-300 ring-1 ring-green-400' : 'bg-blue-500/20 text-blue-300'}`}
      ...
      <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
        ...
        <div className="flex flex-wrap gap-1 justify-center">
          {multiplesA.slice(0, 8).map(m => {
            ...
            return (
              <span key={m} className={`px-2 py-1 rounded text-xs font-bold ${isCommon ? 'bg-yellow-500/30 text-yellow-300 ring-1 ring-yellow-400' : 'bg-white/10 text-gray-400'}`}>
                {m}
              </span>
            );
          })}
      ...
      <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
        ...
        <div className="grid grid-cols-10 gap-1">
          {Array.from({ length: 100 }, (_, i) => i + 1).map(n => (
            <motion.div
              key={n}
              className={`aspect-square rounded flex items-center justify-center text-xs font-bold transition-all cursor-pointer ${
