...
const LightWavesCanvas = lazy(() => import('../../components/canvases/LightWavesCanvas'));
...
export default function LightWaves() {
  const concept = getConceptById('waves-light')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-200">🌈 Light Waves</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-slate-700 border border-slate-500 text-sm font-medium text-slate-200 hover:bg-slate-600 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-indigo-900 to-purple-900 border-b border-slate-600">
        <LightWavesCanvas wavelength={wavelength} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="bg-white rounded-xl border border-slate-100 px-5 py-4 shadow-sm mb-6">
        <label className="block text-sm font-semibold text-slate-700 mb-2">Wavelength: {wavelength} nm</label>
        <input type="range" min="380" max="750" value={wavelength} onChange={e => setWavelength(Number(e.target.value))} className="w-full h-3 rounded-full appearance-none cursor-pointer" style={{ background: 'linear-gradient(to right, #8b5cf6, #3b82f6, #22c55e, #eab308, #f97316, #ef4444)' }} />
        <div className="flex justify-between text-xs text-slate-400 mt-1"><span>Violet</span><span>Blue</span><span>Green</span><span>Yellow</span><span>Red</span></div>
      </div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '🌊', title: 'Light is a Wave', desc: 'Travels through empty space at 300,000 km/s' }, { icon: '🌈', title: 'Visible Spectrum', desc: '380-750 nm — tiny part of EM spectrum' }, { icon: '🔴', title: 'Red = Long', desc: 'Longest visible wavelength (~700nm)' }, { icon: '🟣', title: 'Violet = Short', desc: 'Shortest visible wavelength (~400nm)' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
