...
const FloatSinkCanvas = lazy(() => import('../../components/canvases/FloatSinkCanvas'));
...
export default function FloatSink() {
  const concept = getConceptById('float-sink')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">🚢 Density Test</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-amber-200 text-sm font-medium text-slate-700 hover:bg-amber-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-100">
        <FloatSinkCanvas density={density} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="bg-white rounded-xl border border-slate-100 px-5 py-4 shadow-sm mb-6">
        <label className="block text-sm font-semibold text-slate-700 mb-2">Density: {density.toFixed(1)} g/cm³</label>
        <input type="range" min="0.1" max="3" step="0.1" value={density} onChange={e => setDensity(Number(e.target.value))} className="w-full h-2 bg-gradient-to-r from-green-200 via-yellow-200 to-red-200 rounded-full appearance-none cursor-pointer accent-amber-500" />
        <div className="flex justify-between text-xs text-slate-400 mt-1"><span>🪶 Light (floats)</span><span>💧 Water = 1.0</span><span>🪨 Heavy (sinks)</span></div>
      </div>
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '🚢', title: 'Ships Float', desc: 'Hollow shape = less dense overall' }, { icon: '🪨', title: 'Stones Sink', desc: 'Solid material = more dense than water' }, { icon: '🧊', title: 'Ice Floats', desc: 'Water expands when frozen = less dense' }, { icon: '🥚', title: 'Egg Test', desc: 'Fresh egg sinks, salty water makes it float' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
