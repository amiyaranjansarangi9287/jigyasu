...
const PiCanvas = lazy(() => import('../../components/canvases/PiCanvas'));
...
export default function Pi() {
  const concept = getConceptById('pi')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">🥧 Understanding Pi</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-emerald-200 text-sm font-medium text-slate-700 hover:bg-emerald-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-100">
        <PiCanvas segments={sides} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="bg-white rounded-xl border border-slate-100 px-5 py-4 shadow-sm mb-6">
        <label className="block text-sm font-semibold text-slate-700 mb-2">Polygon Sides: {sides}</label>
        <input type="range" min="3" max="100" value={sides} onChange={e => setSides(Number(e.target.value))} className="w-full h-2 bg-emerald-200 rounded-full appearance-none cursor-pointer accent-emerald-500" />
        <div className="flex justify-between text-xs text-slate-400 mt-1"><span>△ Triangle (3)</span><span>⬡ Hexagon (6)</span><span>⭕ Circle (100)</span></div>
      </div>

      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 p-6">
        <h3 className="font-bold text-emerald-800 mb-4 text-center text-2xl">π ≈ 3.14159...</h3>
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '⭕', title: 'Circumference', desc: 'Distance around the circle' }, { icon: '📏', title: 'Diameter', desc: 'Distance across through the center' }, { icon: '🔢', title: 'Pi = C ÷ D', desc: 'Circumference divided by diameter always = π' }, { icon: '♾️', title: 'Never Ends', desc: 'Pi goes on forever without repeating' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
