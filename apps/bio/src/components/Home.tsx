...
const modules = [
  ...
  { id: 'cell-compare', title: 'Cell Compare', emoji: '🔬', color: 'from-cyan-500 to-blue-500', desc: 'Side-by-side plant vs animal cells', tag: 'Explorer', cat: 'cell', isNew: true },
  { id: 'photo-factory', title: 'Plant Grower', emoji: '🌱', color: 'from-green-500 to-emerald-500', desc: 'Gamified photosynthesis! Grow a plant', tag: 'Game', cat: 'plant', isNew: true },
  { id: 'genetics-breeding', title: 'Pet Breeder', emoji: '🐾', color: 'from-pink-500 to-rose-500', desc: 'Breed virtual pets, see inherited traits', tag: 'Game', cat: 'genetics', isNew: true },
];
  ...
  return (
    <div className="min-h-screen bg-gray-950 pt-11">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-8" style={{ backgroundImage: `url('/images/hero-biology.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(3px)' }} />
        {particles.map(p => (
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-10 pb-6 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-[10px] text-emerald-400/80 font-medium tracking-[0.15em] uppercase">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <div className="text-center">
                ...
                <div className="text-[10px] text-gray-500">{getLevelTitle(progress.level)}</div>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                <div className="text-[10px] text-gray-500 uppercase font-bold">Explored</div>
              ...
              <div className="text-center">
                <div className="text-[10px] text-gray-500 uppercase font-bold">Badges</div>
                <div className="flex justify-center gap-1 mt-1 flex-wrap">
                  ...
                  )) : <span className="text-gray-600 text-xs">Keep exploring!</span>}
                </div>
              </div>
            </div>
          </motion.div>
              ...
              {filteredModules.map((mod) => {
                ...
                return (
                  <motion.button key={mod.id} layout
                    ...
                    {isVisited && <div className="absolute top-1.5 right-1.5 text-[10px]">👁️</div>}
                    <div className="text-xl mb-1">{mod.emoji}</div>
                    <h4 className="text-xs font-bold text-white mb-0.5 group-hover:text-emerald-400 transition-colors leading-tight">{mod.title}</h4>
            ...
            <motion.div key={cat.id}
              ...
              <div className="flex items-center gap-2 mb-2.5 pl-1">
                ...
                {progress && catVisited > 0 && (
                  <span className="text-[9px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded-full">{catVisited}/{catModules.length} explored</span>
                )}
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2.5">
                  ...
                  return (
                    <motion.button key={mod.id}
                      ...
                      <div className="flex items-center gap-1 mb-1.5">
                        <span className="px-1.5 py-px rounded-full text-[8px] font-bold uppercase tracking-wider bg-gray-800 text-gray-500 group-hover:bg-emerald-500/20 group-hover:text-emerald-400">{mod.tag}</span>
                        {mod.isNew && !isVisited && <span className="px-1.5 py-px rounded-full text-[8px] font-bold uppercase bg-yellow-500/20 text-yellow-400 animate-pulse">NEW</span>}
                      </div>
                      <div className="text-xl mb-1">{mod.emoji}</div>
                      <h4 className="text-xs font-bold text-white mb-0.5 group-hover:text-emerald-400 transition-colors leading-tight">{mod.title}</h4>
                      <p className="text-[9px] text-gray-500 leading-tight line-clamp-2">{mod.desc}</p>
      ...
      <div className="border-t border-gray-800 py-3 text-center">
        <p className="text-[9px] text-gray-600">🧬 BioVerse — {modules.length} Interactive Biology Modules • React + Tailwind</p>
      </div>
    </div>
  );
}
