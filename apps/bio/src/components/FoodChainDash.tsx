            ...
            {levels.map((lvl, i) => (
              <motion.button key={lvl.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-xs text-emerald-400 font-bold uppercase">Level {lvl.id}</div>
                    <div className="text-xl font-bold text-white">{lvl.name}</div>
                    <div className="text-xs text-gray-400 mt-1">{lvl.description}</div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-3 bg-gray-900 rounded-xl border border-gray-800 px-4 py-2">
          ...
          <div className="flex items-center gap-4">
            ...
            {invincible && <div className="text-xs text-cyan-400 font-bold animate-pulse">🛡️ Shield!</div>}
          </div>
        </div>
        ...
        <div className="text-center mt-3">
          <button onClick={() => setGameState('menu')} className="text-xs text-gray-500 hover:text-gray-300">← Back to Menu</button>
        </div>
