    ...
    <div className="min-h-screen bg-gray-950 pt-20 pb-10 px-4">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-2">🔋 Cellular Respiration</h2>
          <p className="text-gray-400 text-lg">C₆H₁₂O₆ + 6O₂ → 6CO₂ + 6H₂O + <span className="text-yellow-400 font-bold">~38 ATP</span></p>
        </motion.div>
        ...
        <div className="flex justify-center mb-6">
          <div className="bg-gradient-to-r from-yellow-900/40 to-amber-900/40 rounded-2xl border border-yellow-500/30 px-8 py-4 flex items-center gap-4">
            ...
            <div>
              <div className="text-xs text-yellow-400/70 uppercase font-bold">Total ATP Produced</div>
        ...
        <div className="flex justify-center gap-2 mb-6">
          {stages.map((s, i) => (
            <button key={s.id} onClick={() => { setCurrentStage(i); setShowingStep(0); }}
              ...
              <span className="text-xs opacity-70">+{s.atpProduced}</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                <motion.div key={stage.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">📍 {stage.location}</div>
                    </div>
                    <div className="ml-auto flex items-center gap-2 bg-yellow-500/20 rounded-full px-3 py-1">
                      <Zap className="w-4 h-4 text-yellow-400" />
                      <span className="text-yellow-400 font-bold">{stage.atpProduced} ATP</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-5">
                    <div className="bg-blue-500/10 rounded-xl p-3 border border-blue-500/20">
                      <div className="text-xs text-blue-400 font-bold mb-2">INPUTS →</div>
                    ...
                    <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20">
                      <div className="text-xs text-green-400 font-bold mb-2">→ OUTPUTS</div>
                      {stage.outputs.map(o => <div key={o} className="text-xs text-gray-300 mb-1">• {o}</div>)}
                    </div>
                  </div>

                  {/* Animated steps */}
                  <div className="bg-gray-800/50 rounded-xl p-4">
                    <div className="text-xs text-gray-500 mb-3 uppercase font-bold">Step-by-step Process</div>
                    <div className="space-y-2">
                      {stage.steps.map((step, i) => (
                        <motion.div key={i}
                          ...
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${i <= showingStep || !isPlaying ? 'text-white' : 'bg-gray-700 text-gray-500'}`}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
              <h4 className="text-sm font-bold text-white mb-3">📊 ATP Summary</h4>
              <div className="space-y-2">
                {stages.map((s, i) => (
                  <div key={s.id} className="flex items-center gap-2">
                    ...
                    <span className="flex-1 text-xs text-gray-400">{s.name}</span>
                    <span className={`text-xs font-bold ${currentStage >= i ? 'text-yellow-400' : 'text-gray-600'}`}>+{s.atpProduced}</span>
                  </div>
                ))}
                <div className="border-t border-gray-800 pt-2 mt-2 flex justify-between">
                  <span className="text-sm font-bold text-white">Total</span>
                  <span className="text-sm font-black text-yellow-400">{sumATP} ATP</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-emerald-900/30 to-cyan-900/30 rounded-xl p-4 border border-emerald-500/20">
              <div className="text-xs text-emerald-400 font-bold mb-2">📝 Overall Equation</div>
            ...
            {/* Fun facts */}
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
              <h4 className="text-sm font-bold text-white mb-3">⚡ Energy Facts</h4>
              <ul className="text-xs text-gray-300 space-y-2">
                <li className="flex gap-2"><span className="text-yellow-400">•</span> Your body makes ~40kg of ATP per day</li>
                <li className="flex gap-2"><span className="text-yellow-400">•</span> Brain uses 20% of body's ATP</li>
                <li className="flex gap-2"><span className="text-yellow-400">•</span> 90% of ATP comes from the ETC</li>
                <li className="flex gap-2"><span className="text-yellow-400">•</span> Without O₂, only glycolysis works (2 ATP)</li>
                <li className="flex gap-2"><span className="text-yellow-400">•</span> ATP is recycled ~500 times/day</li>
              </ul>
            </div>

            {/* Fermentation note */}
            <div className="bg-red-500/10 rounded-xl p-4 border border-red-500/20">
              <div className="text-xs text-red-400 font-bold mb-1">🍺 Without Oxygen?</div>
