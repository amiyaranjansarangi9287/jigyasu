          ...
          <motion.div key={selectedCycle.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="flex flex-wrap items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${selectedCycle.type === 'complete' ? 'bg-purple-500/30 text-purple-300' : 'bg-cyan-500/30 text-cyan-300'}`}>
                      {selectedCycle.type} Metamorphosis
                    </span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-gray-500/30 text-gray-300">
              ...
              {selectedCycle.stages.map((s, i) => (
                <div key={s.id} className="flex items-center">
                  <button onClick={() => setSelectedStage(i)}
                    ...
                    <div className={`text-xs font-medium ${selectedStage === i ? 'text-white' : 'text-gray-500'}`}>{s.name}</div>
                  </button>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <motion.div key={stage.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <h5 className="text-xs font-bold text-white mb-2 uppercase tracking-wider">Key Events</h5>
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3">
                {selectedCycle.funFacts.map((fact, i) => (
                  <motion.div key={i}
                    ...
                    className="bg-black/20 rounded-lg p-3 text-xs text-gray-300">
                    {fact}
                  </motion.div>
                ))}
              </div>
