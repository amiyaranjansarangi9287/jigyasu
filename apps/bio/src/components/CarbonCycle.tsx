          ...
          <div className="lg:col-span-3">
            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden border-2 border-emerald-500/20 bg-gradient-to-b from-sky-950/50 via-green-950/30 to-amber-950/40">
              ...
              {Array.from({ length: 8 }).map((_, i) => (
                <motion.div key={i}
                  className="absolute text-[10px] text-blue-300/30 font-mono pointer-events-none"
                  style={{ left: `${15 + Math.random() * 70}%`, top: `${5 + Math.random() * 20}%` }}
            ...
            <div className="mt-4 bg-gray-900 rounded-xl border border-gray-800 p-4 max-h-[220px] overflow-y-auto">
              <h4 className="text-xs font-bold text-white mb-2">🔄 Carbon Flows (click to learn)</h4>
              <div className="grid grid-cols-2 gap-1.5">
                {flows.map((f, i) => {
                  const isHuman = f.process.includes('Combustion');
                  return (
                    <button key={i} onClick={() => { setSelectedFlow(f); setSelectedNode(null); }}
                      className={`text-left px-2 py-1.5 rounded-lg text-[10px] transition-all ${selectedFlow === f ? 'bg-emerald-500/20 border border-emerald-500/30' : isHuman ? 'bg-red-500/10 border border-red-500/20 hover:bg-red-500/20' : 'bg-gray-800/50 border border-transparent hover:bg-gray-800'}`}>
                ...
                <motion.div key={`node-${selectedNode.id}`}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] font-mono text-gray-500">{selectedNode.carbonForm}</div>
                    </div>
                  </div>
                ...
                <motion.div key={`flow-${selectedFlow.process}`}
                  ...
                  {selectedFlow.process.includes('Combustion') && (
                    <div className="mt-3 bg-red-500/10 rounded-lg p-3 border border-red-500/20">
                      <div className="text-xs text-red-400 font-bold">⚠️ Human Impact</div>
                      <p className="text-xs text-gray-300 mt-1">This is the primary driver of climate change. We burn fossil fuels at ~9.5 Gt carbon/year, far exceeding natural carbon cycling.</p>
                    </div>
                  )}
                </motion.div>
              ) : (
