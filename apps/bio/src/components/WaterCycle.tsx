    ...
    <div className="min-h-screen bg-gray-950 pt-20 pb-10 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3 mb-5">
          ...
          <span className="text-xs text-gray-400">Sun Intensity:</span>
            ...
            <div className="relative w-full aspect-[3/2] rounded-2xl overflow-hidden border-2 border-blue-500/20">
              ...
              {vapors.map(v => (
                <motion.div key={`vap-${v.id}`}
                  className="absolute text-[10px] text-blue-300/40 font-mono"
              ...
              {/* Labels */}
              <div className="absolute top-[12%] left-[12%] text-[10px] text-blue-300/60 font-bold">EVAPORATION ↑</div>
              <div className="absolute top-[28%] left-[48%] text-[10px] text-gray-300/60 font-bold">PRECIPITATION ↓</div>
              <div className="absolute bottom-[38%] left-[22%] text-[10px] text-green-300/60 font-bold">TRANSPIRATION</div>
              <div className="absolute bottom-[12%] right-[15%] text-[10px] text-cyan-300/60 font-bold">RUNOFF →</div>
              <div className="absolute bottom-[3%] left-[45%] text-[10px] text-blue-300/40 font-bold">GROUNDWATER</div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-3">
              ...
              <div className="space-y-1.5">
                {processes.map(p => (
                  <button key={p.id}
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl transition-all text-xs ${selectedProcess?.id === p.id ? 'bg-blue-500/15 border border-blue-500/30' : 'bg-gray-800/50 border border-transparent hover:bg-gray-800'}`}>
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
              ...
              <div className="mt-3 bg-blue-500/10 rounded-lg p-2 border border-blue-500/20">
                <p className="text-[10px] text-blue-300">💡 Only 0.61% of all water on Earth is freshwater accessible to humans! The rest is saltwater or locked in ice.</p>
              </div>
            </div>
          </div>
        </div>
