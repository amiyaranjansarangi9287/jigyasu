        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            {selectedPart ? (
              <motion.div key={selectedPart.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                  ...
                  <p className="text-xs text-gray-300">{selectedPart.funFact}</p>
                </div>
              </motion.div>
            ) : (
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
              <h4 className="text-xs font-bold text-white mb-2">🔍 Click to Explore</h4>
              <div className="grid grid-cols-2 gap-1.5">
                {heartParts.map(p => (
                  <button key={p.id} onClick={() => setSelectedPart(p)}
                    className={`text-left px-2 py-1.5 rounded-lg text-xs transition-all ${selectedPart?.id === p.id ? 'bg-gray-700' : 'bg-gray-800/50 hover:bg-gray-800'}`}>
