        ...
        <div className="grid lg:grid-cols-4 gap-5">
              ...
              {ecosystem.length === 0 ? (
                ...
                <div className="space-y-4">
                  {(['producer', 'primary', 'secondary', 'apex', 'decomposer'] as const).map(type => {
                    ...
                    return (
                      <div key={type}>
                        <div className="text-xs font-bold text-gray-500 uppercase mb-2">{typeLabels[type]}</div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                          {orgs.map(org => (
                            <motion.div key={org.id} layout initial={{ scale: 0 }} animate={{ scale: 1 }}
                              ...
                              <div className="text-white text-xs font-bold">{org.name}</div>
                              <div className="text-[10px] text-gray-500 mb-1">{org.description}</div>
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-[10px] text-gray-400 w-6 text-right">{org.population}</span>
                              </div>
                            </motion.div>
                          ))}
                        </div>
              ...
              <div className="mt-4 bg-gray-900 rounded-2xl border border-gray-800 p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {ecosystem.filter(o => o.eats.length > 0).map(predator => (
                    <div key={predator.id} className="bg-gray-800/50 rounded-lg p-2 text-xs">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
              ...
              <div className="text-gray-400 text-center text-xs">{currentWeather.effect}</div>
              {weatherTurnsLeft > 0 && (
                <div className="text-center text-[10px] text-gray-500 mt-1">{weatherTurnsLeft} turns remaining</div>
              )}
            </div>
              ...
              <div className="space-y-1.5 max-h-[250px] overflow-y-auto">
                {messages.map((msg, i) => (
                  <motion.div key={`${i}-${msg}`}
                    ...
                    className="text-xs text-gray-300 bg-gray-800/50 rounded-lg px-2.5 py-1.5">
                    {msg}
                  </motion.div>
                ))}
              </div>
            ...
            {/* Tips */}
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
              <h4 className="text-xs font-bold text-white mb-2">🎯 Strategy Tips</h4>
                      ...
                      {availableOrganisms.filter(o => o.type === type).map(org => {
                        ...
                        return (
                          <button key={org.name} disabled={alreadyAdded}
                            ...
                            className={`flex items-center gap-2 px-2.5 py-2 rounded-xl border text-left text-xs transition-all ${alreadyAdded ? 'opacity-30 cursor-not-allowed border-gray-800' : `${typeColors[type]} hover:brightness-125`}`}>
                            <span className="text-xl">{org.emoji}</span>
                            <div className="min-w-0">
                              <div className="text-white font-bold truncate">{org.name}</div>
                              {org.eats.length > 0 && <div className="text-gray-500 text-[10px] truncate">Eats: {org.eats.join(', ')}</div>}
                            </div>
                          </button>
                        );
                      })}
