        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2">
            <div className="relative bg-gradient-to-b from-sky-950/40 via-green-950/30 to-emerald-950/40 rounded-2xl border border-emerald-500/20 p-6 overflow-hidden min-h-[350px]">
              ...
              {/* Light rays */}
              {lab.lightIntensity > 20 && Array.from({ length: Math.floor(lab.lightIntensity / 15) }).map((_, i) => (
                <motion.div key={i}
                  className="absolute top-0 w-0.5 bg-gradient-to-b from-yellow-400/30 to-transparent"
                  style={{ left: `${20 + i * 12}%`, height: `${30 + Math.random() * 20}%` }}
              ...
              <div className="absolute bottom-4 left-4 right-4 flex gap-3">
                <div className="flex-1 bg-gray-900/80 backdrop-blur rounded-lg p-2">
                  <div className="text-[10px] text-blue-400 font-bold">O₂ Produced</div>
                  <div className="text-lg font-black text-blue-300">{lab.oxygenProduced.toFixed(1)} ml</div>
                </div>
                <div className="flex-1 bg-gray-900/80 backdrop-blur rounded-lg p-2">
                  <div className="text-[10px] text-amber-400 font-bold">Glucose Made</div>
                  <div className="text-lg font-black text-amber-300">{lab.glucoseProduced.toFixed(1)} mg</div>
                </div>
                <div className="flex-1 bg-gray-900/80 backdrop-blur rounded-lg p-2">
                  <div className="text-[10px] text-emerald-400 font-bold">Efficiency</div>
              ...
              {[
                ...
                <div key={ctrl.label} className="bg-gray-900 rounded-xl border border-gray-800 p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-xs font-bold text-gray-300">{ctrl.label}</span>
                    <span className="ml-auto text-xs font-mono" style={{ color: ctrl.color }}>
            ...
            {isRunning && (
              <div className="mt-3 space-y-1">
                {lab.waterLevel < 10 && <div className="text-xs text-red-400 bg-red-500/10 rounded-lg px-3 py-1.5 border border-red-500/20">⚠️ Water level critically low! Photosynthesis slowing down.</div>}
                {lab.co2Level < 10 && <div className="text-xs text-red-400 bg-red-500/10 rounded-lg px-3 py-1.5 border border-red-500/20">⚠️ CO₂ depleted! Calvin cycle cannot fix carbon.</div>}
                {!tempOk && <div className="text-xs text-orange-400 bg-orange-500/10 rounded-lg px-3 py-1.5 border border-orange-500/20">⚠️ Temperature outside optimal range (15-35°C). Enzymes less efficient.</div>}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-1 bg-gray-800 rounded-xl p-1">
              {stageInfo.map((s, i) => (
                <button key={s.id} onClick={() => setActiveStage(i)}
                  className={`flex-1 px-2 py-2 rounded-lg text-xs font-medium transition-all ${activeStage === i ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-white'}`}>
                  {s.emoji}
                </button>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeStage}
                ...
                <p className="text-gray-300 text-xs leading-relaxed mb-3">{stageInfo[activeStage].description}</p>

                {'steps' in stageInfo[activeStage] && (
                  <>
            ...
            <div className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 rounded-2xl border border-green-500/20 p-4">
              <div className="text-xs font-bold text-emerald-400 mb-2">🌍 Why It Matters</div>
