            ...
            <div className="grid lg:grid-cols-2 gap-6">
                  ...
                  {selectedSection === 'root' && (
                    <g>
                      ...
                      {Array.from({ length: 12 }).map((_, i) => {
                        ...
                        const dir = i % 2 === 0 ? 1 : -1;
                        return (
                          <line key={i} x1={200 + dir * 25} y1={y} x2={200 + dir * (50 + Math.random() * 30)} y2={y + (Math.random() - 0.5) * 10}
              ...
              <div className="space-y-4">
                <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">Click structures in the diagram to highlight</div>
                    </div>
                  </div>
                  <p className="text-sm text-gray-300 leading-relaxed mb-4">{part.description}</p>

                  <h4 className="text-xs font-bold text-white mb-2 uppercase tracking-wider">Internal Structures</h4>
                  <div className="space-y-2">
                    {part.structures.map((s, i) => (
                      <motion.button key={i}
                        onClick={() => setHighlightedStructure(highlightedStructure === i ? null : i)}
                        className={`w-full text-left p-2.5 rounded-xl transition-all text-xs ${highlightedStructure === i ? 'bg-emerald-500/15 border border-emerald-500/30' : 'bg-gray-800/50 border border-transparent hover:bg-gray-800'}`}>
                ...
                <div className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 rounded-xl p-4 border border-green-500/20">
                  <div className="flex items-center gap-1.5 text-green-400 font-bold text-xs mb-1">
                    <Info className="w-3 h-3" /> Fun Fact
                  </div>
                  <p className="text-xs text-gray-300 italic">{part.funFact}</p>
                </div>
              </div>
            </div>
          </motion.div>
