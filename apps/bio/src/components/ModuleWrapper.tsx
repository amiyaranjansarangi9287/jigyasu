      ...
      <AnimatePresence>
        {showXP && (
          <motion.div
            ...
            <span className="text-xs opacity-80">New module!</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showLevelUp && (
          <motion.div
            ...
            <motion.div
              ...
              <div className="text-xs text-yellow-400 font-bold uppercase tracking-wider mb-1">Level Up!</div>
      ...
      <div className="sticky bottom-0 z-30 bg-gray-900/95 backdrop-blur-md border-t border-gray-800 pb-safe">
        <div className="max-w-7xl mx-auto px-3 py-2 flex items-center gap-2 sm:gap-3">
          ...
          <button onClick={() => onNavigate('home')}
            className="flex items-center gap-1 text-gray-400 hover:text-white text-xs shrink-0 transition-colors px-2 py-1 rounded-lg hover:bg-gray-800 active:scale-95">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className="text-[10px] text-yellow-400 font-bold">Lv.{progress.level}</span>
              ...
              {connections.related.slice(0, 3).map(relId => {
                ...
                return (
                  <button key={relId} onClick={() => onNavigate(relId)}
                    className={`flex items-center gap-0.5 px-1.5 sm:px-2 py-1 rounded-lg text-[9px] sm:text-[10px] font-medium transition-all active:scale-95 ${isVisited ? 'bg-gray-800 text-gray-500 hover:text-white' : 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25 border border-emerald-500/30'}`}>
                    <span>{mod.emoji}</span>
