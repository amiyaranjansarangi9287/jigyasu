  ...
  const getVisual = () => {
    if (phaseIndex === 0) return ( // Interphase
      <div className="relative w-48 h-48 mx-auto">
        ...
        <div className="absolute top-1/4 right-1/4 text-xs text-gray-400">DNA replicating</div>
      </div>
    );
    if (phaseIndex === 1) return ( // Prophase I
      <div className="relative w-48 h-48 mx-auto">
        ...
        <div className="absolute bottom-12 right-10 text-xs text-yellow-400 bg-yellow-500/20 px-1 rounded">Crossing Over!</div>
      </div>
    );
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl border border-gray-800 p-8 min-h-[280px] flex flex-col items-center justify-center">
      ...
      <div className="mt-6 text-center">
        <div className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${phase.division === 1 ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>
        ...
        <div className="flex justify-center gap-0.5 mb-6 overflow-x-auto px-4 py-2">
          {phases.map((p, i) => (
            <button key={p.id} onClick={() => setCurrentPhase(i)}
              className={`px-2 py-1.5 rounded-lg text-[10px] font-medium transition-all whitespace-nowrap border ${
                i === currentPhase
          ...
          <motion.div key={phase.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">Stage {currentPhase + 1} of {phases.length}</div>
              </div>
            </div>

            <p className="text-gray-300 text-sm leading-relaxed mb-4">{phase.description}</p>

            <h4 className="text-xs font-bold text-white mb-2 uppercase tracking-wider">Key Events</h4>
            ...
            {currentPhase === phases.length - 1 && (
              <div className="bg-gradient-to-r from-green-900/30 to-emerald-900/30 rounded-xl p-4 border border-green-500/20">
                ...
                <p className="text-xs text-gray-300">Each gamete is genetically unique due to crossing over and independent assortment — the basis of genetic diversity!</p>
              </div>
            )}
          </motion.div>
        </div>
