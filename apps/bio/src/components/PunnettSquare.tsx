        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                <div className="text-xs text-blue-400 font-bold mb-2">👨 Parent 1</div>
                <div className="space-y-1.5">
                  {(['homozygous-dominant', 'heterozygous', 'homozygous-recessive'] as Genotype[]).map(g => (
                    <button key={g} onClick={() => setParent1Genotype(g)}
                      ...
                      <span className="text-gray-500 text-xs ml-2">{genotypeLabels[g]}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Parent 2 */}
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                <div className="text-xs text-pink-400 font-bold mb-2">👩 Parent 2</div>
                <div className="space-y-1.5">
                  {(['homozygous-dominant', 'heterozygous', 'homozygous-recessive'] as Genotype[]).map(g => (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">Single gene trait</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-emerald-500/10 rounded-lg p-3 border border-emerald-500/20">
                  <div className="text-xs text-emerald-400 font-bold mb-1">Dominant ({selectedTrait.dominantSymbol})</div>
                  <div className="text-sm text-white">{selectedTrait.dominant}</div>
                </div>
                <div className="bg-purple-500/10 rounded-lg p-3 border border-purple-500/20">
                  <div className="text-xs text-purple-400 font-bold mb-1">Recessive ({selectedTrait.recessiveSymbol})</div>
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
              ...
              {/* Genotype ratio */}
              <div className="mb-4">
                <div className="text-xs text-gray-500 mb-2">Genotype Ratio</div>
                <div className="flex gap-2">
                  {stats.homDom > 0 && (
                    <div className="flex-1 bg-emerald-500/20 rounded-lg p-2 text-center border border-emerald-500/30">
                      ...
                      <div className="text-xs text-gray-400">{stats.homDom}/4 ({(stats.homDom / 4 * 100).toFixed(0)}%)</div>
                    </div>
                  )}
                  {stats.het > 0 && (
                    <div className="flex-1 bg-yellow-500/20 rounded-lg p-2 text-center border border-yellow-500/30">
                      ...
                      <div className="text-xs text-gray-400">{stats.het}/4 ({(stats.het / 4 * 100).toFixed(0)}%)</div>
                    </div>
                  )}
                  {stats.homRec > 0 && (
                    <div className="flex-1 bg-purple-500/20 rounded-lg p-2 text-center border border-purple-500/30">
                      ...
                      <div className="text-xs text-gray-400">{stats.homRec}/4 ({(stats.homRec / 4 * 100).toFixed(0)}%)</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Phenotype ratio */}
              <div className="mb-4">
                <div className="text-xs text-gray-500 mb-2">Phenotype Ratio</div>
                <div className="flex gap-2">
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-emerald-400">{selectedTrait.dominant}</span>
                      <span className="text-xs text-gray-400">{stats.percentDominant.toFixed(0)}%</span>
                    </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-purple-400">{selectedTrait.recessive}</span>
                      <span className="text-xs text-gray-400">{stats.percentRecessive.toFixed(0)}%</span>
                    </div>
                    <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
