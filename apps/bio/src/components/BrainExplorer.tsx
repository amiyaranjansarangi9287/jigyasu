        ...
        <div className="flex gap-3 mb-6 overflow-x-auto pb-2">
          {neuronFacts.map(f => (
            <div key={f.title} className="shrink-0 bg-gray-900 border border-gray-800 rounded-xl px-4 py-2 min-w-[150px]">
              <div className="text-[10px] text-gray-500 uppercase font-bold">{f.title}</div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="relative w-full aspect-[4/3] max-w-[600px] mx-auto rounded-2xl overflow-hidden border-2 border-purple-500/20 bg-gradient-to-br from-indigo-950 via-gray-900 to-purple-950">
              ...
              {showNeuronAnim && Array.from({ length: 35 }).map((_, i) => (
                <motion.div key={i}
                  className="absolute rounded-full bg-purple-400/10"
                  style={{ left: `${8 + Math.random() * 84}%`, top: `${8 + Math.random() * 84}%`, width: 2 + Math.random() * 2, height: 2 + Math.random() * 2 }}
                  animate={{ opacity: [0.05, 0.3, 0.05], scale: [1, 2, 1] }}
                  transition={{ repeat: Infinity, duration: 2 + Math.random() * 4, delay: Math.random() * 3 }}
                />
              ))}

              {/* Synaptic sparks */}
              {showNeuronAnim && Array.from({ length: 5 }).map((_, i) => (
                <motion.div key={`spark-${i}`}
                  className="absolute w-1 h-1 rounded-full bg-yellow-400"
                  style={{ left: `${20 + Math.random() * 60}%`, top: `${15 + Math.random() * 65}%` }}
                  animate={{ opacity: [0, 1, 0], scale: [0, 1.5, 0] }}
                  transition={{ repeat: Infinity, duration: 0.8 + Math.random() * 1.5, delay: i * 1.2, repeatDelay: 2 + Math.random() * 3 }}
                />
              ))}
              ...
              {regions.map(r => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[8px] md:text-[10px] font-bold text-white/80 mt-0.5 leading-tight px-1">{r.name}</span>
                  </motion.button>
                );
              })}
              ...
              {selected ? (
                <motion.div key={selected.id}
                  ...
                  <div className="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 rounded-xl p-3 mb-3 border border-purple-500/20">
                    ...
                    <p className="text-gray-300 text-xs italic">{selected.funFact}</p>
                  </div>

                  <div className="bg-red-500/10 rounded-xl p-3 border border-red-500/20">
                    <div className="text-xs text-red-400 font-bold mb-1">⚠️ If Damaged</div>
                    <p className="text-xs text-gray-300">{selected.damage}</p>
                  </div>
                </motion.div>
              ) : (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div className="bg-gray-800/30 rounded-xl p-3 text-left">
                    <h4 className="text-xs font-bold text-gray-400 mb-2">💡 Quick Facts</h4>
