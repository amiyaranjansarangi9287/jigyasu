...
export default function TermsOfService() {
  ...
  useEffect(() => {
    document.title = t('terms.page_title', 'Terms of Service — Jigyasu');
    const meta = document.querySelector('meta[name="description"]');
    if (meta) {
      meta.setAttribute('content', t('terms.meta_description', 'Jigyasu Terms of Service. Please read these terms carefully before using our platform.'));
    }
  }, [t]);

  return (
    <main className="max-w-4xl mx-auto py-12 px-6 pb-24 text-slate-800">
      <h1 id="terms-title" className="text-4xl font-black mb-8 text-sky-600">{t('terms_of_service', 'Terms of Service')}</h1>
      
      <div className="space-y-6 text-lg">
        <section aria-labelledby="terms-acceptance">
          <h2 id="terms-acceptance" className="text-2xl font-bold mb-3">{t('terms_acceptance', 'Acceptance of Terms')}</h2>
          <p>
        ...
        <section aria-labelledby="terms-acceptable-use">
          <h2 id="terms-acceptable-use" className="text-2xl font-bold mb-3">{t('terms_acceptable_use', 'Acceptable Use Policy')}</h2>
        ...
        <section aria-labelledby="terms-content">
          <h2 id="terms-content" className="text-2xl font-bold mb-3">{t('terms_content', 'Content Guidelines')}</h2>
          <p>
        ...
        <section aria-labelledby="terms-liability">
          <h2 id="terms-liability" className="text-2xl font-bold mb-3">{t('terms_liability', 'Limitation of Liability')}</h2>
          <p>
        ...
        <section aria-labelledby="terms-termination">
          <h2 id="terms-termination" className="text-2xl font-bold mb-3">{t('terms_termination', 'Termination')}</h2>
          <p>
        ...
        <section aria-labelledby="terms-governing-law">
          <h2 id="terms-governing-law" className="text-2xl font-bold mb-3">{t('terms_governing_law', 'Governing Law')}</h2>
          <p>
        ...
        <section aria-labelledby="terms-contact">
          <h2 id="terms-contact" className="text-2xl font-bold mb-3">{t('terms_contact', 'Contact Information')}</h2>
          <p>
            {t('terms_contact_text', 'For any questions about these Terms, please contact us at legal@jigyasu.app.')}
          </p>
          <p className="mt-2 text-sm text-slate-600">
            {t('terms_general_contact', 'For general inquiries: contact@jigyasu.app')}
          </p>
        </section>
      </div>
    </main>
