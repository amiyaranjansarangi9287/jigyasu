...
export default function CampWeeksPreview({ onSelectWeek, getWeekStatus }: CampWeeksPreviewProps) {
  ...
  const getStatusBadge = (weekId: string) => {
    ...
    if (status === 'in-progress') {
      return (
        <span className="px-2 py-1 rounded-full bg-orange-500 text-white text-sm font-medium">
          In Progress
        </span>
      );
    }
  ...
  const getButtonText = (weekId: string) => {
    const status = getWeekStatus(weekId);
    if (status === 'completed') return 'Do Again';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-orange-100 dark:bg-orange-900/30 rounded-full px-4 py-2 mb-4">
            ...
            <span className="text-sm font-bold text-orange-600 dark:text-orange-400 uppercase tracking-widest">Project Weeks</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">
            Structured Project Journeys
          </h2>
          <p className="text-gray-600 dark:text-gray-300 text-lg max-w-2xl mx-auto">
            Follow our curated 5-day project plans. Perfect for school breaks, weekends, or whenever you need structured hands-on fun!
          </p>
        </div>
              ...
              <div className={`relative p-6 bg-gradient-to-r ${week.color}`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-white">{week.name}</h3>
                      <p className="text-white/80 text-sm">5 days of activities</p>
                    </div>
                  </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-2 mb-4 overflow-x-auto no-scrollbar">
                  {week.days.map((day) => (
                    ...
                    >
                      <span className="text-sm text-gray-500 dark:text-gray-400">Day {day.day}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    {week.ageRange === 'all' ? 'All ages' : `Ages ${week.ageRange}`}
                  </span>
                  <button
        ...
        >
          <p className="text-gray-600 dark:text-gray-300 mb-4">
            Complete all 4 Project Weeks to earn the <span className="font-bold text-orange-500">🏆 Summer Champion</span> badge!
          </p>
        </div>
      </div>
    </section>
