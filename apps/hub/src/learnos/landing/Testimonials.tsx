  ...
  return (
    <section id="voices" className="mx-auto max-w-6xl px-5 py-16">
      <div className="mx-auto max-w-2xl text-center">
        <span className="text-sm font-bold uppercase tracking-wider text-brand">{t('testimonials.coming_soon', 'Coming Soon')}</span>
        <h2 className="mt-2 font-display text-3xl font-bold text-slate-900 md:text-4xl">
          {t('testimonials.share_exp', 'Share Your Experience')}
        </h2>
        <p className="mt-4 text-slate-600">
          {t('testimonials.building_with_love', 'We\'re building Jigyasu with love. Once we launch, we\'ll feature real stories from learners and parents here.')}
        </p>
      </div>
      ...
      <div className="mt-14 overflow-hidden rounded-[2rem] bg-gradient-to-br from-brand via-orange-500 to-rose-500 p-8 text-center text-white shadow-2xl shadow-orange-300/50 md:p-12">
        <h3 className="font-display text-3xl font-bold md:text-4xl">{t('testimonials.ready', 'Ready to learn something today?')}</h3>
        <p className="mx-auto mt-3 max-w-xl text-orange-50">
          {t('testimonials.no_signup', 'No sign-up. No download required. Open the app and start in your language in under 10 seconds.')}
        </p>
