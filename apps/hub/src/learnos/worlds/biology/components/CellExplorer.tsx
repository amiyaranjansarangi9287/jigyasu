...
const cytoplasmParticles = Array.from({ length: 40 }, (_, i) => ({
  left: `${10 + ((i * 19) % 80)}%`,
  top: `${10 + ((i * 31) % 80)}%`,
  size: 1 + ((i * 7) % 30) / 10,
  x: (i % 5 - 2) * 4,
  y: ((i * 2) % 5 - 2) * 4,
  duration: 3 + ((i * 13) % 50) / 10,
  delay: ((i * 7) % 30) / 10,
}));
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 relative">
            <div className="relative w-full aspect-square max-w-[620px] mx-auto rounded-3xl overflow-hidden border-2 border-emerald-500/30 bg-gradient-to-br from-emerald-950 via-teal-950 to-cyan-950">
              ...
              {cytoplasmParticles.map((particle, i) => (
                <motion.div key={i}
                  className="absolute rounded-full bg-emerald-400/10"
                  style={{ left: particle.left, top: particle.top, width: particle.size, height: particle.size }}
                  animate={{ opacity: [0.05, 0.25, 0.05], scale: [1, 1.8, 1], x: [0, particle.x], y: [0, particle.y] }}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-1">
                        {selected.cellTypes.map(t => (
                          <span key={t} className="px-2 py-0.5 rounded-full text-sm font-bold uppercase bg-gray-800 text-gray-400">
