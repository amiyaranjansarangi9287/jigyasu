        ...
        <div className="flex justify-center gap-1 mb-6 overflow-x-auto pb-2 px-2 -mx-2">
          {phases.map((p, i) => (
            <button key={p.id} onClick={() => handlePhaseChange(i)}
              className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-full text-sm sm:text-sm font-medium transition-all whitespace-nowrap shrink-0 ${
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl border border-gray-800 p-4 sm:p-6">
              ...
              <div className="flex justify-center gap-2 sm:gap-3 mt-4">
                ...
                <button onClick={() => setAutoPlay(!autoPlay)}
                  className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full font-medium text-sm sm:text-sm flex items-center gap-1.5 active:scale-95 transition-transform ${autoPlay ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'}`}>
              ...
              <div className="text-center mt-2 text-gray-600 text-sm sm:text-sm">Phase {currentPhase + 1} of {phases.length}</div>
            </div>
          </div>
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4 sm:p-5">
              <div className="flex items-center gap-2 sm:gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-sm sm:text-sm font-medium" style={{ color: phase.color }}>Stage {currentPhase + 1}</div>
                </div>
              </div>

              <p className="text-gray-300 text-sm sm:text-sm leading-relaxed mb-4">{phase.description}</p>

              <h4 className="text-sm sm:text-sm font-bold text-white mb-2 uppercase tracking-wider">What's Happening</h4>
              <ul className="space-y-1.5 sm:space-y-2">
                {phase.details.map((detail, i) => (
                  <motion.li key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.05 }}
                    className="flex items-start gap-2 text-[11px] sm:text-sm text-gray-300">
                    <span className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center text-[8px] sm:text-sm font-bold text-white mt-0.5 shrink-0"
              ...
              {currentPhase === phases.length - 1 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <p className="text-gray-300 text-sm sm:text-sm">Two identical daughter cells created, each with a complete copy of DNA!</p>
                </motion.div>
              )}
            </div>
          </motion.div>
