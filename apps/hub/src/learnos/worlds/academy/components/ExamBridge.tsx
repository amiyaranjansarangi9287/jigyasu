...
export function ExamBridge({ questions, onComplete }: Props) {
  ...
  if (done) { const c = ans.filter((a, i) => questions[i].correctIndex !== undefined && a === questions[i].correctIndex).length; return <div className="p-5 text-center"><div className="text-4xl mb-3">{c === questions.length ? '🎯' : '📝'}</div><p className="text-white font-bold text-lg">{c}/{questions.length} <Trans i18nKey="auto.exambridge.correct">correct</Trans></p></div>; }
  ...
  return (
    <div className="bg-slate-900 rounded-2xl overflow-hidden">
      <div className="px-4 py-2.5 flex items-center gap-2" style={{ backgroundColor: `${BC[q.board]}22` }}>
        ...
        <span className="ml-auto text-slate-500 text-sm">{q.marks} <Trans i18nKey="auto.exambridge.marks">marks ·</Trans> {q.topic}</span>
      </div>
      <div className="p-4">
        ...
        {!q.options && <div className="bg-slate-800/50 rounded-xl p-3 mb-4 border border-slate-700 text-center"><p className="text-slate-400 text-sm"><Trans i18nKey="auto.exambridge.structured_question_attempt_in">Structured question — attempt in your notebook</Trans></p><button onClick={() => setShowMS(true)} className="mt-2 text-indigo-400 text-sm underline"><Trans i18nKey="auto.exambridge.show_mark_scheme">Show mark scheme</Trans></button></div>}
        <AnimatePresence>{(showMS || (picked && q.options)) && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-slate-800 rounded-xl p-3 mb-4 border border-slate-700"><p className="text-slate-400 text-sm font-bold uppercase mb-1"><Trans i18nKey="auto.exambridge.mark_scheme">Mark Scheme</Trans></p><p className="text-slate-300 text-sm leading-relaxed">{q.markscheme}</p></motion.div>
        )}</AnimatePresence>
