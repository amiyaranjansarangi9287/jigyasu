  ...
  return (
    <AcademyShell module="climate-systems">
      <div className="min-h-screen bg-slate-950 p-5 pb-24">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4"><div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 text-center"><p className="text-slate-500 text-sm">{t('academy.modules.ClimateSystems.txt_CO', 'CO₂')}</p><p className="text-white text-2xl font-bold">{co2} ppm</p></div><div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 text-center"><p className="text-slate-500 text-sm">{t('academy.modules.ClimateSystems.txt_Warming', 'Warming')}</p><p className={`text-2xl font-bold ${tempRise >= 2 ? 'text-red-400' : 'text-cyan-400'}`}>{tempRise}°C</p></div></div>
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 mb-4"><div className="flex justify-between text-sm mb-2"><span className="text-slate-400">{t('academy.modules.ClimateSystems.spn_COLevel', 'CO₂ Level')}</span><span className="text-white">{co2}</span></div><input type="range" min="280" max="600" value={co2} onChange={(e) => setCo2(Number(e.target.value))} className="w-full" /><div className="mt-3"><p className="text-slate-500 text-sm mb-1">Arctic ice cover: {iceCover}%</p><div className="h-3 bg-slate-800 rounded-full"><div className="h-full bg-cyan-400 rounded-full" style={{ width: `${iceCover}%` }} /></div></div></div>
