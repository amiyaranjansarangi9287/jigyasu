  ...
  return (
    <AcademyShell module="essay-architect">
      <div className="min-h-screen bg-slate-950 p-5 pb-24">
        ...
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800 mb-4"><div className="flex justify-between text-sm mb-2"><span className="text-slate-400 font-bold">{t('academy.modules.EssayArchitect.spn_FlowScore', 'Flow Score')}</span><span className="text-white font-bold">{score}<Trans i18nKey="auto.essayarchitect.100">/100</Trans></span></div><div className="h-2 bg-slate-800 rounded-full"><div className="h-full bg-violet-500 rounded-full" style={{ width: `${score}%` }} /></div></div>
        ...
        <button onClick={() => setShowModel(!showModel)} className="w-full mt-4 py-3 bg-slate-800 text-slate-300 rounded-xl font-bold">{showModel ? 'Hide' : 'Show'} <Trans i18nKey="auto.essayarchitect.model_structure">Model Structure</Trans></button>
        {showModel && <div className="mt-3 bg-violet-950/30 border border-violet-900/40 rounded-xl p-3 text-sm text-violet-200"><Trans i18nKey="auto.essayarchitect.thesis_evidence_analysis_count">Thesis → Evidence → Analysis → Counter → Rebuttal → Conclusion. Every block must earn its place.</Trans></div>}
