  ...
  return (
    <AcademyShell module="trig-identities">
      <div className="min-h-screen bg-slate-950 p-5 pb-24">
        ...
        <div className="bg-slate-900 rounded-3xl p-6 border border-slate-800 mb-4 text-center"><p className="text-cyan-400 text-sm font-bold uppercase mb-2">{t('academy.modules.TrigIdentities.txt_Identity', 'Identity')}</p><p className="text-white text-2xl font-mono font-bold">{identity.statement}</p></div>
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 mb-4"><p className="text-slate-400 text-sm font-bold uppercase mb-3">{t('academy.modules.TrigIdentities.txt_Proofsteps', 'Proof steps')}</p><div className="space-y-2">{identity.proof.map((step, i) => <div key={i} className="bg-slate-800 rounded-xl p-3 text-sm text-slate-300"><span className="text-cyan-400 font-bold mr-2">{i+1}.</span>{step}</div>)}</div></div>
        {verified && <div className="bg-green-950/30 rounded-xl p-3 border border-green-900/30 text-green-300 text-sm mb-4"><Trans i18nKey="auto.trigidentities.verified_numerically_and_geome">✓ Verified numerically and geometrically.</Trans></div>}
        <button onClick={verify} className="w-full py-4 bg-cyan-700 text-white rounded-2xl font-bold">{t('academy.modules.TrigIdentities.btn_VerifyIden', 'Verify Identity')}</button>
      </div>
    </AcademyShell>
  );
}
