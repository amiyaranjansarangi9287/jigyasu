// src/worlds/academy/data/academyContent.ts
import type { AcademyModuleMetadata, AcademyModule } from '../types/academy.types';

export const ACADEMY_MODULES: AcademyModuleMetadata[] = [
...
export const BEAUTY_MOMENTS: Record<AcademyModule, string> = {
...
export function calculateTrigPoint(deg: number) {
  const rad = (deg * Math.PI) / 180;
  return { angle: deg, radians: rad, sinValue: Math.round(Math.sin(rad) * 10000) / 10000, cosValue: Math.round(Math.cos(rad) * 10000) / 10000, tanValue: Math.abs(Math.tan(rad)) > 1000 ? null : Math.round(Math.tan(rad) * 10000) / 10000, quadrant: (deg >= 0 && deg < 90 ? 1 : deg >= 90 && deg < 180 ? 2 : deg >= 180 && deg < 270 ? 3 : 4) as 1|2|3|4 };
}

export const SPECIAL_ANGLES = [0, 30, 45, 60, 90, 120, 135, 150, 180, 210, 225, 240, 270, 300, 315, 330, 360];

export const TRIG_MEMORY_AIDS = [
...
export interface CurveDefinition { id: string; label: string; expression: string; fn: (x: number) => number; derivative: (x: number) => number; domain: [number, number]; }
...
export const ELECTROLYSIS_SETUPS = [
