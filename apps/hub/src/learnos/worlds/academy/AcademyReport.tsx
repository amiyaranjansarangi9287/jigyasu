            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                {ACADEMY_MODULES.map((module) => {
                  ...
                  return <div key={module.id} className="flex items-center gap-3 px-3 py-2 bg-slate-800 rounded-xl"><span className="text-lg">{module.emoji}</span><span className="text-slate-300 text-sm flex-1 truncate">{module.title}</span>{depth ? <span className="text-sm font-bold" style={{ color: DEPTH_CONFIG[depth].color }}>{DEPTH_CONFIG[depth].symbol} {DEPTH_CONFIG[depth].label}</span> : <span className="text-slate-700 text-sm"><Trans i18nKey="auto.academyreport.not_started">Not started</Trans></span>}</div>;
                })}
              </div>
