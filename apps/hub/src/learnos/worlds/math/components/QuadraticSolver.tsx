  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-white mb-2"><Trans i18nKey="auto.quadraticsolver.quadratic_solver">📈 Quadratic Solver</Trans></h2>
        <p className="text-purple-300 text-lg"><Trans i18nKey="auto.quadraticsolver.explore_roots_vertex_discrimin">Explore roots, vertex, discriminant, and the quadratic formula.</Trans></p>
      </div>

      <div className="flex justify-center gap-2 mb-6">
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${mode === 'explore' ? 'bg-blue-500/30 text-blue-300 border border-blue-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => setMode('explore')}><Trans i18nKey="auto.quadraticsolver.explore">🔍 Explore</Trans></button>
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${mode === 'challenge' ? 'bg-purple-500/30 text-purple-300 border border-purple-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => { setMode('challenge'); setChallenge(makeQuadraticChallenge()); }}><Trans i18nKey="auto.quadraticsolver.challenge">🎯 Challenge</Trans></button>
      </div>
        ...
        {mode === 'challenge' ? (
          <motion.div key="challenge" className="max-w-lg mx-auto" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className={`rounded-3xl p-6 border-2 ${feedback === 'correct' ? 'bg-green-500/10 border-green-500/40' : feedback === 'hint' ? 'bg-white/5 border-white/10' : 'bg-white/5 border-white/10'}`}>
              <div className="flex justify-between mb-4"><span className="text-yellow-400 font-bold">⭐ {mastery}</span><span className="text-gray-400 text-sm"><Trans i18nKey="auto.quadraticsolver.find_both_roots">Find both roots</Trans></span></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {challenge.options.map((opt) => <motion.button key={opt} className={`py-3 rounded-xl text-lg font-bold ${feedback === 'correct' && opt === challenge.roots ? 'bg-green-500 text-white' : feedback ? 'bg-white/5 text-gray-500' : 'bg-white/10 text-white hover:bg-white/20 border border-white/20'}`} whileHover={!feedback ? { scale: 1.05 } : {}} whileTap={!feedback ? { scale: 0.95 } : {}} onClick={() => answerChallenge(opt)} disabled={!!feedback}><Trans i18nKey="auto.quadraticsolver.x">x =</Trans> {opt}</motion.button>)}
              </div>
              {feedback === 'correct' && <p className="text-green-400 text-center font-bold mt-4"><Trans i18nKey="auto.quadraticsolver.correct_roots">✅ Correct roots!</Trans></p>}
              {feedback === 'hint' && <p className="text-sky-400 text-center font-bold mt-4"><Trans i18nKey="auto.quadraticsolver.try_factoring_or_the_formula">Try factoring or the formula.</Trans></p>}
            </div>
          </motion.div>
        ) : (
          <motion.div key="explore" className="grid grid-cols-1 lg:grid-cols-2 gap-6" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="space-y-4">
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10 flex justify-center">
                <svg width={graph.size} height={graph.size} className="bg-black/20 rounded-xl">
                  ...
                  {Math.abs(calc.vertexX) <= 10 && Math.abs(calc.vertexY) <= 10 && <g><circle cx={graph.sx(calc.vertexX)} cy={graph.sy(calc.vertexY)} r="5" fill="#f59e0b" /><text x={graph.sx(calc.vertexX) + 8} y={graph.sy(calc.vertexY) - 8} fill="#f59e0b" fontSize="10"><Trans i18nKey="auto.quadraticsolver.vertex">vertex</Trans></text></g>}
                </svg>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
                ...
                <p className="text-gray-500 text-sm mt-2"><Trans i18nKey="auto.quadraticsolver.here_x">Here: x = (</Trans>{-b} ± √{calc.d.toFixed(2)}) / {2 * a}</p>
              </div>
            </div>
          </motion.div>
        )}
