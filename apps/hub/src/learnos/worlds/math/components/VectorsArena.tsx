  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-white mb-2"><Trans i18nKey="auto.vectorsarena.vectors_arena">🏹 Vectors Arena</Trans></h2>
        <p className="text-purple-300 text-lg"><Trans i18nKey="auto.vectorsarena.visualize_vector_addition_magn">Visualize vector addition, magnitude, and dot product!</Trans></p>
      </div>

      <div className="flex justify-center gap-2 mb-6">
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${mode === 'explore' ? 'bg-blue-500/30 text-blue-300 border border-blue-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => setMode('explore')}><Trans i18nKey="auto.vectorsarena.explore">🔍 Explore</Trans></button>
        <button className={`px-4 py-2 rounded-xl font-bold text-sm ${mode === 'challenge' ? 'bg-purple-500/30 text-purple-300 border border-purple-400/50' : 'bg-white/5 text-gray-400'}`} onClick={() => { setMode('challenge'); setChallenge(makeChallenge()); }}><Trans i18nKey="auto.vectorsarena.challenge">🎯 Challenge</Trans></button>
      </div>
        ...
        {mode === 'challenge' ? (
          <motion.div key="ch" className="max-w-lg mx-auto" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className={`rounded-3xl p-6 border-2 ${feedback === 'correct' ? 'bg-green-500/10 border-green-500/40' : feedback === 'hint' ? 'bg-white/5 border-white/10' : 'bg-white/5 border-white/10'}`}>
              ...
              {feedback === 'correct' && <p className="text-green-400 font-bold text-center mt-4"><Trans i18nKey="auto.vectorsarena.correct">✅ Correct!</Trans></p>}
              {feedback === 'hint' && <p className="text-sky-400 font-bold text-center mt-4"><Trans i18nKey="auto.vectorsarena.answer">Answer:</Trans> {challenge.answer}</p>}
            </div>
          </motion.div>
        ) : (
          <motion.div key="ex" className="grid grid-cols-1 lg:grid-cols-2 gap-6" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="space-y-4">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <h4 className="text-blue-400 font-bold mb-2"><Trans i18nKey="auto.vectorsarena.vector_a">Vector A</Trans></h4>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.x">x</Trans></label><input type="range" min="-5" max="5" value={ax} onChange={e => setAx(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{ax}</p></div>
                  <div><label className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.y">y</Trans></label><input type="range" min="-5" max="5" value={ay} onChange={e => setAy(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{ay}</p></div>
                </div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <h4 className="text-sky-400 font-bold mb-2"><Trans i18nKey="auto.vectorsarena.vector_b">Vector B</Trans></h4>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.x">x</Trans></label><input type="range" min="-5" max="5" value={bx} onChange={e => setBx(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{bx}</p></div>
                  <div><label className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.y">y</Trans></label><input type="range" min="-5" max="5" value={by} onChange={e => setBy(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{by}</p></div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20 text-center"><p className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.a_b">A + B</Trans></p><p className="text-green-400 font-bold">({sum.x}, {sum.y})</p></div>
                <div className="bg-purple-500/10 rounded-xl p-3 border border-purple-500/20 text-center"><p className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.a_b">A · B</Trans></p><p className="text-purple-400 font-bold">{dot}</p></div>
                <div className="bg-blue-500/10 rounded-xl p-3 border border-blue-500/20 text-center"><p className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.a">|A|</Trans></p><p className="text-blue-400 font-bold">{magA.toFixed(2)}</p></div>
                <div className="bg-orange-500/10 rounded-xl p-3 border border-orange-500/20 text-center"><p className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.b">|B|</Trans></p><p className="text-sky-400 font-bold">{magB.toFixed(2)}</p></div>
              </div>
              <div className="bg-yellow-500/10 rounded-xl p-3 border border-yellow-500/20 text-center"><p className="text-gray-400 text-sm"><Trans i18nKey="auto.vectorsarena.angle_between">Angle between</Trans></p><p className="text-yellow-400 font-bold">{angle.toFixed(1)}°</p></div>
            </div>
