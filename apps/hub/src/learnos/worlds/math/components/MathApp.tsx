    ...
    <div className="min-h-screen relative">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 hidden sm:block">Where numbers come alive! ✨</p>
            </div>
          </motion.div>
      ...
      >
        <Breadcrumbs items={[
          ...
          { label: 'LearnOS', href: '/', emoji: '🧠' },
          { label: 'Math Kingdom', emoji: '🧙‍♂️' },
          ...(activeTab !== 'home' ? [{ label: tabs.find(t => t.id === activeTab)?.label || 'Module', emoji: tabs.find(t => t.id === activeTab)?.emoji }] : [])
        ]} />
        <AnimatePresence mode="wait">
          ...
          {activeTab !== 'home' && (
            <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
              <Suspense fallback={<LoadingCharacter message="Preparing your math adventure..." />}>
      ...
      <footer className="relative z-10 text-center py-6 border-t border-white/5">
        <p className="text-gray-600 text-sm">Made with 💜 for math wizards of all levels ✨</p>
        <p className="text-gray-700 text-sm mt-1">50+ activities · 350+ challenges · Ages 5–18+</p>
      </footer>
    </div>
  );
}
...
function HomePage({ onNavigate }: { onNavigate: (tab: Tab) => void }) {
  const features = [
    { tab: 'daily' as Tab, emoji: '🎯', title: 'Daily Challenge', description: 'A fresh challenge every day! Build your streak and sharpen your skills with a new problem each morning!', color: 'from-yellow-500/20 to-amber-500/20', border: 'border-yellow-500/30', buttonColor: 'from-yellow-600 to-amber-600' },
    { tab: 'explorers' as Tab, emoji: '🧭', title: 'Explorers Guild', description: '20 visual adventures! From counting coins to chess strategy, sports stats, and engineering math.', color: 'from-violet-500/20 to-purple-500/20', border: 'border-violet-500/30', buttonColor: 'from-violet-600 to-purple-600' },
    { tab: 'map' as Tab, emoji: '🗺️', title: 'Adventure Map', description: 'Explore the Math Kingdom! Travel through Addition Forest, Subtraction Caves, and more. Solve challenges to earn stars!', color: 'from-green-500/20 to-emerald-500/20', border: 'border-green-500/30', buttonColor: 'from-green-600 to-emerald-600' },
    { tab: 'visualizer' as Tab, emoji: '📊', title: '3D Math Visualizer', description: 'See numbers come alive! Watch addition build towers, multiplication create grids, and division split groups — all in 3D!', color: 'from-blue-500/20 to-cyan-500/20', border: 'border-blue-500/30', buttonColor: 'from-blue-600 to-cyan-600' },
    { tab: 'skills' as Tab, emoji: '🎓', title: 'Skills Academy', description: 'Build core math foundations! Master times tables, grow number bonds, and explore fractions with pizza!', color: 'from-rose-500/20 to-pink-500/20', border: 'border-rose-500/30', buttonColor: 'from-rose-600 to-pink-600' },
    { tab: 'advanced' as Tab, emoji: '🔥', title: 'Advanced Math', description: 'Algebra, trig, vectors, matrices, logs, calculus, complex numbers, statistics, probability — plus SAT/ACT practice and Math Olympiad!', color: 'from-red-500/20 to-orange-500/20', border: 'border-red-500/30', buttonColor: 'from-red-600 to-orange-600' },
    { tab: 'game' as Tab, emoji: '🎮', title: 'Number Crunch Quest', description: 'Race against the clock! Solve math puzzles as fast as you can. Build combos and reach legendary wizard status!', color: 'from-pink-500/20 to-rose-500/20', border: 'border-pink-500/30', buttonColor: 'from-pink-600 to-rose-600' },
    { tab: 'pattern' as Tab, emoji: '🧩', title: 'Patterns & Memory', description: 'Discover hidden patterns in number sequences — each type has a whimsical theme! Also play Memory Match to train your mind!', color: 'from-amber-500/20 to-orange-500/20', border: 'border-amber-500/30', buttonColor: 'from-amber-600 to-orange-600' },
  ];
    ...
    <div className="space-y-10">
      ...
      <div className="text-center py-8 sm:py-12 relative">
        ...
        <motion.h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold mb-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <span className="bg-gradient-to-r from-purple-400 via-pink-400 via-amber-400 to-green-400 bg-clip-text text-transparent">Welcome to the</span>
          <br />
          <span className="bg-gradient-to-r from-yellow-400 via-orange-400 to-red-400 bg-clip-text text-transparent">Math Kingdom!</span>
        </motion.h2>
        ...
        <motion.p className="text-gray-400 text-lg sm:text-xl max-w-2xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
          A whimsical world where numbers dance, patterns hide, and every math problem is an adventure waiting to be solved! 🌟
        </motion.p>
        ...
        <motion.div className="flex justify-center gap-4 sm:gap-8 mt-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          {[
            ...
            { label: 'Challenges', value: '350+', emoji: '⚔️' },
      ...
      <motion.div className="bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-amber-500/10 rounded-2xl border border-purple-500/20 p-6 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
        <h3 className="text-lg font-bold text-white mb-3">✨ Did You Know?</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white/5 rounded-xl p-4">
            <span className="text-3xl">🔢</span>
            <p className="text-sm text-gray-300 mt-2"><strong className="text-white">111,111,111 × 111,111,111</strong> = 12,345,678,987,654,321 — a perfect palindrome!</p>
          </div>
          <div className="bg-white/5 rounded-xl p-4">
            <span className="text-3xl">🍕</span>
            <p className="text-sm text-gray-300 mt-2">A pizza that has radius <strong className="text-white">"z"</strong> and height <strong className="text-white">"a"</strong> has volume = Pi × z × z × a</p>
          </div>
          <div className="bg-white/5 rounded-xl p-4">
            <span className="text-3xl">♾️</span>
            <p className="text-sm text-gray-300 mt-2">The word <strong className="text-white">"hundred"</strong> comes from the old Norse word <strong className="text-white">"hundrath"</strong> meaning 120!</p>
          </div>
        </div>
      </motion.div>
    </div>
