          ...
          <motion.div key="ex" className="max-w-3xl mx-auto space-y-4" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="bg-white/5 rounded-2xl p-5 border border-white/10">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div><label className="text-gray-400 text-sm">First term (a₁)</label><input type="range" min="1" max="10" value={a1} onChange={e => setA1(Number(e.target.value))} className="w-full accent-blue-500" /><p className="text-white font-bold text-center">{a1}</p></div>
                {seqType === 'arithmetic' ? (
                  <div><label className="text-gray-400 text-sm">Common diff (d)</label><input type="range" min="-5" max="10" value={d} onChange={e => setD(Number(e.target.value))} className="w-full accent-green-500" /><p className="text-white font-bold text-center">{d}</p></div>
                ) : (
                  <div><label className="text-gray-400 text-sm">Common ratio (r)</label><input type="range" min="-3" max="5" value={r} onChange={e => setR(Number(e.target.value))} className="w-full accent-purple-500" /><p className="text-white font-bold text-center">{r}</p></div>
                )}
                <div><label className="text-gray-400 text-sm">Terms (n)</label><input type="range" min="3" max="15" value={n} onChange={e => setN(Number(e.target.value))} className="w-full accent-orange-500" /><p className="text-white font-bold text-center">{n}</p></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="bg-green-500/10 rounded-xl p-4 border border-green-500/20 text-center"><p className="text-gray-400 text-sm">Sum of {n} terms</p><motion.p key={sum} className="text-green-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{Math.round(formulaSum * 100) / 100}</motion.p></div>
              <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20 text-center"><p className="text-gray-400 text-sm">nth term (a{toSup(n)})</p><motion.p key={nthTerm} className="text-blue-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{Math.round(nthTerm * 100) / 100}</motion.p></div>
              <div className="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20 text-center"><p className="text-gray-400 text-sm">Average</p><motion.p key={sum / n} className="text-purple-400 font-bold text-2xl" initial={{ scale: 0.5 }} animate={{ scale: 1 }}>{(sum / n).toFixed(2)}</motion.p></div>
            </div>
