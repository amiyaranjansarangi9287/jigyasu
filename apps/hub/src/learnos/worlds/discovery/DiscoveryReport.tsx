      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-slate-950/95 flex items-end justify-center" onClick={onClose}>
        <motion.div initial={{ y: 80 }} animate={{ y: 0 }} exit={{ y: 80 }} className="bg-slate-900 rounded-t-3xl w-full max-w-md max-h-[88vh] overflow-y-auto border-t border-slate-700" onClick={e => e.stopPropagation()}>
          <div className="sticky top-0 bg-slate-900 px-5 pt-5 pb-3 border-b border-slate-800 flex items-center justify-between">
            <div><h2 className="text-lg font-extrabold text-white">Discovery Report</h2><p className="text-slate-500 text-sm">Your constellation of knowledge</p></div>
          ...
          {report && (
            <div className="p-5 space-y-4">
              ...
              <div className="bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-2xl p-4 border border-indigo-700/30 flex items-center justify-between">
                <div><p className="text-indigo-400 text-sm font-bold uppercase">Wonder Score</p><p className="text-white text-sm mt-1">{report.rabbitHoles} rabbit holes · {report.connectionsFound} connections</p></div>
              ...
              <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50"><p className="text-slate-400 text-sm italic">"The measure of intelligence is the ability to change."</p><p className="text-slate-600 text-sm mt-2">— Lumo</p></div>
              <button onClick={onClose} className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl min-h-[52px]">Return to Constellation</button>
            </div>
          )}
        </motion.div>
      </motion.div>
