    ...
    <LabShell module="lever-explorer" subject="physics">
      <div className="min-h-screen bg-stone-50 flex flex-col p-6 pb-24">
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-stone-200 mb-6">
          <h2 className="font-bold text-lg"><Trans i18nKey="auto.leverexplorer.lever_explorer">Lever Explorer ⚙️</Trans></h2>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm mb-6" style={{ height: '140px' }}>
          <div className="relative w-full h-full flex items-end">
            ...
            {/* Weights */}
            <div className="absolute bottom-14 left-2 text-center"><span className="text-3xl">{t('lab.modules.LeverExplorer.spn_', '🏋️')}</span><p className="text-sm font-bold">{leftW}<Trans i18nKey="auto.leverexplorer.kg">kg</Trans></p></div>
            <div className="absolute bottom-14 right-2 text-center"><span className="text-3xl">{t('lab.modules.LeverExplorer.spn_', '📦')}</span><p className="text-sm font-bold">{rightW}<Trans i18nKey="auto.leverexplorer.kg">kg</Trans></p></div>
            {/* Torque display */}
            <div className="absolute top-0 left-2 text-sm text-slate-400"><Trans i18nKey="auto.leverexplorer.t">T =</Trans> {leftTorque}</div>
            <div className="absolute top-0 right-2 text-sm text-slate-400"><Trans i18nKey="auto.leverexplorer.t">T =</Trans> {rightTorque}</div>
          </div>
        </div>
