    ...
    <LabShell module="fraction-kitchen" subject="math">
      <div className="flex flex-col h-screen bg-red-50 overflow-auto pb-24 p-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm mb-6 border border-red-100">
          <div className="flex items-center gap-4 mb-4"><span className="text-4xl">{r.emoji}</span><div><h2 className="text-xl font-bold">{r.name}</h2><p className="text-sm text-slate-500">Original servings: {r.baseServings}</p></div></div>
          <div className="bg-red-50 p-4 rounded-xl font-bold text-red-800">Target: Feed {r.targetServings} people (×{f})</div>
        </div>
        <div className="space-y-4">
          {r.ingredients.map(i => (
            <div key={i.name} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
              <div><p className="font-bold">{i.name}</p><p className="text-sm text-slate-400">Base: {i.baseAmount} {i.unit}</p></div>
