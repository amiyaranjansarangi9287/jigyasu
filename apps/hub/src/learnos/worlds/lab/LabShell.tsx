      ...
      <AnimatePresence>
        {showBreak && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-slate-900/95 flex flex-col items-center justify-center p-8">
            <div className="text-center max-w-sm"><div className="text-6xl mb-4">🦚</div><h2 className="text-2xl font-bold text-white mb-2"><Trans i18nKey="auto.labshell.great_work_today">Great Work Today</Trans></h2><p className="text-slate-400 text-sm mb-8"><Trans i18nKey="auto.labshell.you_ve_been_doing_science_for_">You've been doing science for 20 minutes. Your brain deserves a rest.</Trans></p>
              <div className="flex gap-3">
                <button onClick={dismissBreak} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl"><Trans i18nKey="auto.labshell.5_more_minutes">5 More Minutes</Trans></button>
                <button onClick={() => navigate('/lab')} className="flex-1 py-3 bg-slate-700 text-white font-bold rounded-xl"><Trans i18nKey="auto.labshell.go_to_lab_home">Go to Lab Home</Trans></button>
              </div>
            </div>
          </motion.div>
        )}
