...
const ElectricityCanvas = lazy(() => import('../../components/canvases/ElectricityCanvas'));
...
export default function Electricity() {
  const concept = getConceptById('electricity')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">⚡ Electric Circuit</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-blue-200 text-sm font-medium text-slate-700 hover:bg-blue-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-100">
        <ElectricityCanvas circuitComplete={true} bulbs={circuit === 0 ? 1 : circuit === 1 ? 2 : 3} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {['💡 Simple', '🔋 Series', '🔌 Parallel'].map((c, i) => (
          <button key={i} onClick={() => setCircuit(i)} className={`px-4 py-3 rounded-xl font-semibold transition-all ${circuit === i ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:border-blue-300'}`}>{c}</button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '🔋', title: 'Battery', desc: 'Provides the push (voltage) for electrons' }, { icon: '💡', title: 'Bulb', desc: 'Electrons heat filament → light' }, { icon: '🔌', title: 'Complete Circuit', desc: 'Loop must be closed for current to flow' }, { icon: '🚫', title: 'Open Circuit', desc: 'Break in loop = no current = no light' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
