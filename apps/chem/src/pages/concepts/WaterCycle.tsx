...
const WaterCycleCanvas = lazy(() => import('../../components/canvases/WaterCycleCanvas'));
...
export default function WaterCycle() {
  const concept = getConceptById('water-cycle')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">💧 Water Cycle</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-amber-200 text-sm font-medium text-slate-700 hover:bg-amber-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-100">
        <WaterCycleCanvas stage={stage} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {['☀️ Evaporation', '☁️ Condensation', '🌧️ Precipitation', '🌊 Collection'].map((s, i) => (
          <button key={i} onClick={() => setStage(i)} className={`px-4 py-3 rounded-xl font-semibold transition-all ${stage === i ? 'bg-amber-500 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:border-amber-300'}`}>{s}</button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '☀️', title: 'Evaporation', desc: 'Sun heats water → turns to vapor → rises' }, { icon: '☁️', title: 'Condensation', desc: 'Vapor cools → forms clouds' }, { icon: '🌧️', title: 'Precipitation', desc: 'Clouds get heavy → rain/snow falls' }, { icon: '🌊', title: 'Collection', desc: 'Water flows to rivers/oceans → cycle repeats' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
