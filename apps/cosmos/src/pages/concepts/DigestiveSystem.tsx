...
const DigestiveCanvas = lazy(() => import('../../components/canvases/DigestiveCanvas'));
...
export default function DigestiveSystem() {
  const concept = getConceptById('digestive-system')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">🫁 Digestive System</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-red-200 text-sm font-medium text-slate-700 hover:bg-red-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-red-50 to-pink-50 border-b border-red-100">
        <DigestiveCanvas stage={stage} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {['👄 Mouth', '📏 Esophagus', '🫗 Stomach', '🔄 Small Int.', '💧 Large Int.', '🚪 Exit'].map((s, i) => (
          <button key={i} onClick={() => setStage(i)} className={`px-3 py-2 rounded-xl font-semibold transition-all text-sm ${stage === i ? 'bg-red-500 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:border-red-300'}`}>{s}</button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-2xl border border-red-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '👄', title: 'Mouth', desc: 'Teeth break food, saliva starts digestion' }, { icon: '🫗', title: 'Stomach', desc: 'Strong acids break food into liquid' }, { icon: '🔄', title: 'Small Intestine', desc: '6m long — nutrients absorbed here' }, { icon: '💧', title: 'Large Intestine', desc: 'Removes water, waste exits' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
