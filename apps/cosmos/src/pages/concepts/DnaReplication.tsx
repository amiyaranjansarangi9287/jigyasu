...
const DnaReplicationCanvas = lazy(() => import('../../components/canvases/DnaReplicationCanvas'));
...
export default function DnaReplication() {
  const concept = getConceptById('dna-replication')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">🧬 DNA Replication</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-green-200 text-sm font-medium text-slate-700 hover:bg-green-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100">
        <DnaReplicationCanvas unzipProgress={stage / 3} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {['🧬 Double Helix', '✂️ Unzip', '🔗 Base Pair', '📋 Copy'].map((s, i) => (
          <button key={i} onClick={() => setStage(i)} className={`px-4 py-3 rounded-xl font-semibold transition-all ${stage === i ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:border-green-300'}`}>{s}</button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '🧬', title: 'Double Helix', desc: 'Twisted ladder shape — two strands' }, { icon: '🔗', title: 'Base Pairs', desc: 'A-T and C-G — always match this way' }, { icon: '✂️', title: 'Unzip', desc: 'Enzyme splits DNA down the middle' }, { icon: '📋', title: 'Perfect Copy', desc: 'Each half serves as template for new strand' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
