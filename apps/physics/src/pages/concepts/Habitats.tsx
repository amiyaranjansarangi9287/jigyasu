...
const HabitatsCanvas = lazy(() => import('../../components/canvases/HabitatsCanvas'));
...
export default function Habitats() {
  const concept = getConceptById('habitats')!
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <CanvasWithNarrator headerContent={<><span className="font-bold text-slate-700">🦁 Animal Habitats</span><button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1.5 rounded-full bg-white border border-green-200 text-sm font-medium text-slate-700 hover:bg-green-50 transition-colors">{isPlaying ? '⏸ Pause' : '▶ Play'}</button></>} currentNarration={currentNarration} isNarrating={isNarrating} headerClassName="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100">
        <HabitatsCanvas habitat={habitat} isPlaying={isPlaying} />
      </CanvasWithNarrator>

      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {['🌲 Forest', '🌊 Ocean', '🏜️ Desert', '❄️ Arctic'].map((h, i) => (
          <button key={i} onClick={() => setHabitat(i)} className={`px-4 py-3 rounded-xl font-semibold transition-all ${habitat === i ? 'bg-green-500 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:border-green-300'}`}>{h}</button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ icon: '🌲', title: 'Forest', desc: 'Trees provide food and shelter for many animals' }, { icon: '🌊', title: 'Ocean', desc: 'Covers 70% of Earth — home to whales, fish, coral' }, { icon: '🏜️', title: 'Desert', desc: 'Hot and dry — animals adapt to survive with little water' }, { icon: '❄️', title: 'Arctic', desc: 'Freezing cold — thick fur and fat keep animals warm' }].map((f, i) => (<div key={i} className="flex items-start gap-3 bg-white/70 rounded-xl p-3"><span className="text-xl">{f.icon}</span><div><p className="font-semibold text-slate-700 text-sm">{f.title}</p><p className="text-xs text-slate-500">{f.desc}</p></div></div>))}
        </div>
      </div>
    </div>
  )
