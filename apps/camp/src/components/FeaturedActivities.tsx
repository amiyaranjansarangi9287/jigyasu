...
export default function FeaturedActivities({
  ...
  const getStatusBadge = (activityId: string) => {
    ...
    if (status === 'completed') {
      return (
        <span className="absolute top-3 left-3 px-2 py-1 rounded-full bg-green-500 text-white text-xs font-medium flex items-center gap-1">
    ...
    if (status === 'in-progress') {
      return (
        <span className="absolute top-3 left-3 px-2 py-1 rounded-full bg-orange-500 text-white text-xs font-medium">
          In Progress
        </span>
      );
    }
